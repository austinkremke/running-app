...
- **Milestone 07 (in progress):** team play — Phase 1–2 shipped: creation/management (`create_team` L10-gated, role-checked RPCs, leader auto-succession) + `team_rank` with real Team tab stats (`get_team_overview` / `list_top_teams`). Next: team matchmaking with top-N contributor scoring. See [milestones/07-team-play.md](milestones/07-team-play.md).
