...
- **Milestone 09 (planned):** wearable ingest via **HealthKit** — one integration covers Apple Watch **and** Garmin, no Garmin partner approval. Verified firsthand: Garmin writes workouts + HR to HealthKit but **not** GPS routes, so route is a *quality* signal (unlocks Pace Distribution / Climbing Analysis), **not** an XP/match eligibility gate. Imports earn XP only at the **Verified** tier — gating is deliberately **permissive** (`wasUserEntered=false` + HR series present + plausible pace/HR + recent); `HKSource`/`HKDevice`/sample-density are **recorded in `import_metadata` but not gated**, so the policy can be tightened from real abuse patterns later. Enforced **server-side** in `award_run_xp` / `credit_match_activity`. Blocking type change: `ActivityRecord.latitude/longitude` are required today and must become nullable for route-less imports. See [milestones/09-wearable-integration.md](milestones/09-wearable-integration.md).
- **Premium analytics (in progress):** three premium run-detail cards (Pace Distribution → Climbing Analysis → Heart Rate Analysis), all owner-only and ungated (no paywall UI yet — full analysis shown to everyone until that lands), all built on shared `paceSegments.ts` (smoothed, grade-adjusted, moving-time-only pace windows). **Pace Distribution** — personalized Recovery/Easy/Workout/Hard pace ranges computed from the raw per-point track (`activityTrackService`), cached per-user in `user_pace_profiles` (`paceProfileService`, duration-weighted percentiles over ~90 days, not a simple average), classified per-run in `paceDistributionService`. **Climbing Analysis** — terrain distribution, significant-climb detection with gap-bridging, per-climb pacing (thirds/fade/strong-finish), downhill classification, route classification (`climbingAnalysisService`). **Heart Rate Analysis** — `heartRateSamples.ts` cleans raw HR (range/flatline/isolated-spike filtering) before `heartRateAnalysisService` builds personalized zones (from this run's own observed sustained max — no stored user max/resting/threshold HR exists yet, clearly labeled as an estimate), zone distribution, cardiovascular-profile classification, HR drift, pace/HR relationship, and sustained high-HR segments. **Note:** no HR sensor integration exists (`ActivityRecord.heartRateBpm` is never populated by phone-GPS tracking), so this card renders its "unavailable" state on all real runs today — it's real, working analysis waiting on a wearable/HealthKit data source. No cross-run history, matched-climb, or interval-lap analysis for any of the three cards yet.
