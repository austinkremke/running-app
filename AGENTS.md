...
- **Me tab stats:** `OverallStatsSection` now has range tabs (`OverallStatsRangeTabs` — All Time/Last Week/Last Month/Last Year; `fetchProfileOverallStats(userId, since?)` filters by `started_at`). New `CompetitiveStatsSection` (`competitiveStatsService.fetchCompetitiveStats`) shows overall W-L-T record plus avg distance/points per match — outcome per match determined by summing all `match_participants` points on the runner's side vs every other side (works for solo 1v1 and team sides alike), avg points is the runner's own `points` column (not side total). Always all-time, not range-filtered.
