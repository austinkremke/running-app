...
- **Milestone 07 (in progress):** team play — Phase 1–3 shipped: creation/management (`create_team` L10-gated, role-checked RPCs, leader auto-succession), `team_rank` + real Team tab stats (`get_team_overview` / `list_top_teams`), and team matchmaking queue (`enqueue_team_matchmaking`, leader/co-leader only, pairs by rating; `TeamMatchTab` de-mocked, lineup picker removed). Next: Phase 4 top-N scoring + `finalize_team_match`. See [milestones/07-team-play.md](milestones/07-team-play.md).
