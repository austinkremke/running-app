-- Fix finalize_team_match: match_points_for_activity takes (numeric, numeric),
-- but activities.distance_meters is double precision and duration_seconds is
-- integer. Postgres does not implicitly cast double precision -> numeric
-- (only as an assignment cast), so the unqualified call inside the top-5
-- scorer subquery threw "function match_points_for_activity(double
-- precision, integer) does not exist" every time finalize ran, aborting the
-- whole transaction (status update, Elo, completion payload all rolled
-- back) — the match silently stayed 'active' forever with no drawer and no
-- rating change. Add explicit ::numeric casts.
