-- Allow teams to use a custom uploaded logo image instead of the
-- icon/accent combo. Reuses the existing public "avatars" storage bucket
-- (writes are already scoped to the uploading user's own folder — team
-- logo uploads are gated at the RPC layer by leader/co-leader role, same
-- as every other team-edit field, not by storage path ownership).
...
begin
  ...
  update public.teams
    set name = coalesce(nullif(v_name, ''), name),
        ...
        -- empty string clears a custom logo back to the icon/accent avatar
