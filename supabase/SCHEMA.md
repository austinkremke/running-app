...
follows                         — follower_id, followed_id (instant one-directional follow, no request/approval)
...
**Friend requests → follows:** `20260808000001_follow_system.sql` — `friendships(user_id, friend_user_id)` renamed to `follows(follower_id, followed_id)`; following someone is now instant and one-directional (like Twitter/Instagram), no request/approval step. `add_friend`/`remove_friend` (bidirectional insert/delete) dropped for `follow_user`/`unfollow_user` (single-row insert/delete) — `follows_select_all` RLS is public-read (`using (true)`), like team rosters/leaderboards elsewhere, so follower/following lists and counts don't need a security-definer RPC. `are_friends(a, b)` kept its name (every existing caller — `can_view_feed_post`, `send_solo_match_challenge`, `achievement_metric`'s `friend_count` case — references it) but now means "does `a` follow `b`"; it was already single-directional so `can_view_feed_post`'s friends-audience check needed no logic change. The 1v1 challenge gate ("must be mutual friends") is now "must follow them" — any-direction follow is enough, a deliberate product choice, not a side effect. `enqueue_friend_activity_notifications` (friends-audience feed post → notify) had to flip its query direction — the old bidirectional data made "people I friended" and "people who friended me" the same set, but `follows` is genuinely directional, so it now explicitly selects *followers* of the poster, not who the poster follows. Dropped entirely: `friend_requests` table, `send_friend_request`/`respond_to_friend_request`/`cancel_friend_request`/`get_friend_notifications`/`has_friend_notifications` RPCs, and the `friend_requests`-category push pipeline (trigger + `notification_events`/`notification_preferences` column), replaced by a `new_follower` category firing on every `follows` insert (`enqueue_new_follower_notification`). The friend-request branch of the in-app actionable-notification drawer (`useTeamNotifications`/`NotificationCenterDrawer`, previously shared with team invites/join requests) was removed outright — there's nothing to accept/decline anymore. `add_friend` achievement id/`criteria_type` left unchanged (avoids touching historical `user_achievements` rows) but its display copy was updated ("Follow Someone" / "Follow your first runner."). Client: `followService.ts` (replaces `friendService.ts`/`friendRequestService.ts`), `useFollows.ts` (replaces `useFriends.ts`, no more `isPending`/pending state at all).

**Real per-country solo ranking:** `20260811000001_country_ranking.sql` — the Me tab and solo matchmaking screen's "country rank" had always actually been the *global* rank number reused with a device-locale flag next to it (documented as a known placeholder in `deviceRegion.ts`/`soloRankPositionService.ts` since it was added) — there was no stored country to scope a ranking by. Added `profiles.country_code` (nullable `text`, `^[A-Z]{2}$` check, partial index on non-null rows) — self-serve and optional, set via a new `set_my_country(p_country_code)` RPC, never forced during onboarding (a deliberate product decision — country rank is a nice-to-have, not worth adding signup friction for). New `get_solo_rank_position(p_rating, p_country_code default null)` RPC replaces two direct `player_rank` count queries the client used to run itself (no RPC existed before this) — `p_country_code` omitted/null still gives the global count (join against `profiles` costs nothing extra at this scale), set gives a country-scoped one. Client: `soloRankPositionService.ts` now exports only `fetchCountryRankPosition(rating, countryCode)` (the old global-only `fetchSoloRankPosition` was deleted — nothing called it once both the Me tab and solo matchmaking screen switched to the real country-scoped fetch using the viewer's own stored `country_code`, falling back to hiding the rank slot entirely when unset rather than showing a fake number). New `CountryPickerDrawer` (searchable list, `src/utils/countryOptions.generated.ts` — 250 ISO-3166-1 alpha-2 codes with English names, generated from the `country-list` npm package cross-referenced against `flagRegistry.generated.ts` so every pickable country has a matching flag) wired into a new Settings → Profile → "Country" row (`updateMyCountry` in `profileService.ts`). `deviceRegionCode()` (`expo-localization`) is no longer read as the rank-scope source anywhere — it's kept only as the picker's first-open default suggestion. Scope: solo only — team country ranking would need the same pattern on `teams` (a `country_code` set by the team leader) plus a team-scoped variant of the RPC; deliberately not built here, since "whose country represents an 8-person team" is a separate product question. The `MatchTeamSummaryCard`/`TeamMatchTab` team country-rank slot is therefore untouched and still shows the same device-locale-flag-plus-global-number placeholder solo used to.
