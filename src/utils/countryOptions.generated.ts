/**
 * Static list of ISO-3166-1 alpha-2 country codes + English display names,
 * scoped to exactly the codes that have artwork in `flagRegistry.generated.ts`
 * (so every entry a user can pick is guaranteed to have a matching flag) and
 * filtered to real countries a person would select as their own — supranational
 * / unofficial entries (EU, IC, AC, TA, XA, XC, XO) are excluded from this list,
 * though their flags remain in the registry for other uses. Names come from the
 * `country-list` package; Kosovo (XK) has no ISO entry there, so it's named
 * manually — it's a widely used de facto code.
 *
 * Regenerate the same way (see the generation script in this file's git history)
 * if the flag registry's country set ever changes.
 */
